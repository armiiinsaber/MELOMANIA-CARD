...
export async function POST(req: Request) {
  ...
  if (!e || !u) return NextResponse.json({ ok:false, reason:'missing_params' }, { status: 400 });

  const { data: profile, error: pErr } = await supabaseAdmin
    .from('profiles').select('id').eq('email', e).eq('username', u).maybeSingle();
  ...
  const { data: pass, error: passErr } = await supabaseAdmin
    .from('passes').select('qr_token,status').eq('user_id', profile.id).eq('event_slug', slug)
